...
class Beat:
    ...
    def set_duration(self, dur, recurse=True):
        print "setting duration of", self, "now to", dur
        print "I have a duration:", self.has_duration()
    ...
    def enact_local_constraints(self):
        parent = self.parent
        print "Here's some local constraint info"
        print self
        print self.has_duration()
        print self.size
        print "whose parent is", parent
        print parent.constrained() if parent else "constrained? wuh?"

        if self.has_duration():
            for link in self.linked_beats:
                print "propagating known duration to", link
                link.set_duration(self._duration)

            if self.size and parent and parent.constrained():
